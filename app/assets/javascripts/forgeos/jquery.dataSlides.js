...
(function($) {
	 ...
	 * Purpose:  Store information for dataSlides to access globally about other instances
	 * Scope:    jQuery.fn.dataSlideExt
	 */
	...
	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 * dataSlides prototype
	 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	
	/*
	 * Function: dataSlide
	 * Purpose:  dataSlides information
	 * Returns:  -
	 * Inputs:   object:oInit - initalisation options for the table
	 */
	$.fn.dataSlide = function( oInit )
	{
		/*
		 ...
		 */
		var _aoSettings = $.fn.dataSlideSettings;
		...
		function classSettings ()
		{
			...
			/*
			 * Variable: sInstance
			 * Purpose:  Unique idendifier for each instance of the dataSlides object
			...
			/*
			 * Variable: oFeatures
			 * Purpose:  Indicate the enablement of key dataSlide features
			 * Scope:    jQuery.dataSlide.classSettings
			 */
			this.oFeatures = {
				"bPaginate": true,
			...
			/*
			 * Variable: oLanguage
			 * Purpose:  Store the language strings used by dataSlides
			 * Scope:    jQuery.dataSlide.classSettings
		 ...
		 *   in effect prevnts dataSlides from drawing it (hence deleting it) - it could be restored
      ...
      $(nNewCell).addClass('td')
		...
		this.fnSetColumnVis = function ( iCol, bShow )
		{
			...
			if ( bShow )
			{
				...
				else
				{
					...
					nTrHead.insertBefore( anTheadTh[iCol], $(nTrHead).children('.th')[iBefore] );
					if ( nTrFoot )
					{
						nTrFoot.insertBefore( anTfootTh[iCol], $(nTrFoot).children('.th')[iBefore] );
					}

					for ( i=0, iLen=oSettings.aoData.length ; i<iLen ; i++ )
					{
						nTd = oSettings.aoData[i]._anHidden[iCol];
						oSettings.aoData[i].nTr.insertBefore( nTd, oSettings.aoData[i].nTr.children('.td')[iBefore] );
					}
				}
			...
			else
			{
				...
				for ( i=0, iLen=oSettings.aoData.length ; i<iLen ; i++ )
				{
					nTd = oSettings.aoData[i].nTr.children('.td')[ iVisCol ];
		 ...
		 * dataSlides object, providing a rather nice way to allow plug-in API functions. Note that
		...
		function _fnAddColumn( oSettings, oOptions, nTh )
		{
			oSettings.aoColumns[ oSettings.aoColumns.length++ ] = {
				...
				"nTh": nTh ? nTh : document.createElement('div'),
				"nTf": null
			};
      ...
      $(newTr).addClass('tr');
		...
		function _fnGatherData( oSettings )
		{
			...
			for ( i=0 ; i<oSettings.aoColumns.length ; i++ )
			{
				...
				if ( bAutoType || bRender || bClass || !bVisible )
				{
					...
					for ( j=0 ; j<iLoop ; j++ )
					{
						var nCellNode = oSettings.aoData[j].nTr.children('.td')[ i-iCorrector ];
		...
		function _fnDraw( oSettings )
		{
			...
			else
			{
				...
				anRows[ 0 ] = document.createElement( 'div' );
				$(anRows[ 0 ]).addClass('tr');
		...
		function _fnAddOptionsHtml ( oSettings )
		{
			...
			/* 
			 * All dataSlides are wrapped in a div - this is not currently optional - backwards
		 ...
		 * Inputs:   object:oS - dataSlides settings object
		 */
		 ...
		 * Notes:    This function makes use of the dataSlides plugin objct _oExt
		 ...
		 * Inputs:   node:nTable - table we are using as a dataSlide
		 */
		 ...
		 * Purpose:  Get the column ordering that dataSlides expects
		...
		function _fnSaveState ( oSettings )
		{
			...
			sValue += "}";
			_fnCreateCookie( "SpryMedia_dataSlides_"+oSettings.sInstance, sValue,
		 ...
		 * Returns:  -
		 * Inputs:   object:oSettings - dataSlides settings object
		 *           object:oInit - dataSlides init object so we can override settings
		 */
		function _fnLoadState ( oSettings, oInit )
		{
			...
			var oData;
			var sData = _fnReadCookie( "SpryMedia_dataSlides_"+oSettings.sInstance );
			if ( sData !== null && sData !== '' )
			{
		...
		function _fnGetUniqueThs ( nThead )
		{
			...
			for ( i=0, iLen=nTrs.length ; i<iLen ; i++ )
			{
				...
				{
					if ( nTrs[i].childNodes[j].nodeName == "DIV")
					{
				...
				for ( j=0, jLen=nTds.length ; j<jLen ; j++ )
				{
					...
					if ( !iColspan || iColspan===0 || iColspan===1 )
					{
						...
						aLayout[i][iColumnShifted] = (nTds[j].nodeName=="DIV") ? TDELEM : nTds[j];
		...
		return this.each(function()
		{
			...
			{
				$(this).append('<div class="thead"></div>');
			}
			
			if ( $(this).children('.tbody').length === 0 )
			{
				$(this).append('<div class="tbody"></div>');
			}
