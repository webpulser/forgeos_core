  ...
  def display_visual(object_name, picture, name = 'picture')
    content_tag :div, :class => 'fieldset open' do
      ...
      content_tag(:div, :class =>'option-panel-content') do
        picture.nil? ? label = t("#{object_name}.visual.add").capitalize : label = t(:change, :scope => [object_name, name]).capitalize
        ...
        content_tag(:div, :class => 'grid_9', :id => "#{name}-picture") do
          content_tag(:ul, :class => 'sortable') do
            unless picture.nil?
              content_tag(:li) do
                link_to_function('', "$(this).parents('li').remove(); $('##{name}').val('null');", :title => t('media.picture.destroy.confirm').capitalize, :class => 'big-icons trash') +
        ...
        content_tag(:div, :class => 'grid_9', :id => 'visuals-picture') do
          content_tag(:ul, :class => 'sortable') do
            pictures.collect{ |picture|            
              content_tag(:li) do
                link_to_function('', "$(this).parents('li').remove();", :title => t('media.picture.destroy.confirm').capitalize, :class => 'big-icons trash') +
