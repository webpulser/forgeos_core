...
class Admin::UsersControllerTest < ActionController::TestCase
  ...
  test "should get show" do
    ...
    get :show, :id => people(:user).id, :use_route => :forgeos_core
    assert_response :success
    assert_equal people(:user), assigns(:user)
  ...
  test "should get edit" do
    ...
    get :edit, :id => people(:user).id, :use_route => :forgeos_core
    assert_response :success
    assert_equal people(:user), assigns(:user)
  ...
  test "should put update" do
    ...
    put :update, :id => people(:user).id, :user => {
      :lastname => 'test',
      :firstname => 'test',
      :email => 'test@forgeos.com',
      :password => 'forgeos',
    ...
    assert_response :success
    assert_equal people(:user), assigns(:user)
    assert_equal 'test', people(:user).reload.lastname
    assert_template 'admin/users/edit'
  end
  ...
  test "should put update with invalid record" do
    ...
    assert_equal people(:user), assigns(:user)
    assert !assigns(:user).valid?, "user is valid and should not be"
    assert_not_equal 'test', people(:user).reload.lastname
    assert_template 'admin/users/edit'
  end
  ...
  test "should delete destroy" do
    ...
    assert_difference 'User.count', -1 do
      delete :destroy, :id => people(:user).id, :use_route => :forgeos_core
    end
  ...
  test "should delete destroy in js" do
    admin_login_to('admin/users', 'destroy')
    assert_difference 'User.count', -1 do
      delete :destroy, :format => 'js', :id => people(:user).id, :use_route => :forgeos_core
    end
  ...
  test "should put activate with active user" do
    ...
    put :activate, :id => people(:user).id, :use_route => :forgeos_core

    assert_equal people(:user), assigns(:user)
    assert !people(:user).reload.active?
    assert_redirected_to '/admin/users'
  end

  test "should put activate with unactive user" do
    admin_login_to('admin/users', 'activate')
    people(:user).update_attribute(:active, false)
    @request.env['HTTP_REFERER'] = 'http://test.host/admin/users'
    put :activate, :id => people(:user).id, :use_route => :forgeos_core
  ...
  test "should not put activate with active user" do
    admin_login_to('admin/users', 'activate')
    # invalidate people(:user) record
    people(:user).email = ''
    people(:user).save(:validate => false)
    @request.env['HTTP_REFERER'] = 'http://test.host/admin/users'
    put :activate, :id => people(:user).id, :use_route => :forgeos_core
  ...
  test "should get export_newsletter with Internet Explorer" do
    admin_login_to('admin/users', 'export_newsletter')
